<script lang="ts">
    ...
    async function handleDelete(name: string) {
        if (!aws.ses) return;
        if (!(await confirmDialog({ message: `Delete configuration set ${name}?`, confirmText: "Delete", destructive: true }))) return;
        try {
            loading = true;
            await aws.ses.send(
...
<Modal bind:open={showCreateModal} title="Create Configuration Set">
    <div class="space-y-4 p-4 text-gray-300">
        ...
        <div class="flex justify-end pt-2">
            ...
            >
                {#if creating}<span class="animate-spin"><Icon path={mdiLoading} class="animate-spin" size={14} /></span>{/if} Create
            </button>
        </div>
    </div>
</Modal>
