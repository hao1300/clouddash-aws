<script lang="ts">
    ...
    let rawDimensions = $derived.by(() => {
        ...
        try {
            // Try parsing as JSON first (from our new links)
            return JSON.parse(dimensionsStr);
        } catch {
            // Fallback for manual or legacy links: Name=Val, Name2=Val2
    ...
    <div class="flex-1 min-h-0 flex flex-col gap-4 overflow-hidden">
        ...
        >
            ...
            {:else}
                    ...
                    >
                        ...
                        >
                            ...
                            {#each metricChartData.path.split(" L ") as pt, i}
                                <circle
                                    cx={i === 0
                                        ? pt.substring(2).split(",")[0]
                                        : pt.split(",")[0]}
                                    cy={i === 0
                                        ? pt.substring(2).split(",")[1]
                                        : pt.split(",")[1]}
                                    r="2.5"
                                    fill="#60A5FA"
                                />
                ...
                <div
                    class="mt-4 text-[10px] text-gray-500 font-mono truncate border-t border-gray-800 pt-3"
                >
