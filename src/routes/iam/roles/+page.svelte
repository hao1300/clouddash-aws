<script lang="ts">
    ...
    import CreateRoleModal from "$lib/components/iam/CreateRoleModal.svelte";
...
<div class="h-full relative overflow-hidden flex flex-col">
    ...
    <div class="px-6 py-3 border-b border-gray-800 bg-gray-900/50 flex justify-between items-center shrink-0 {error ? 'mt-8' : ''}">
        <h2 class="text-sm font-bold text-gray-300 uppercase tracking-widest">IAM Roles</h2>
    ...
    />
    <CreateRoleModal bind:show={showCreateModal} onSaved={() => { history = []; loadRoles(); }} />
</div>
</div>
</div>
