        ...
        tabs={[
            {
                id: "messages",
                label: "Messages",
                href: `/sqs/queue/${encodeURIComponent(queueName)}/messages?url=${encodeURIComponent(queueUrl)}`,
            },
            {
                id: "metrics",
                label: "Metrics",
                href: `/sqs/queue/${encodeURIComponent(queueName)}/metrics?url=${encodeURIComponent(queueUrl)}`,
            },
        ]}
        activeTab="messages"
    />
    <div
        class="flex-1 flex flex-col p-4 bg-gray-950 text-white overflow-hidden relative"
    >
        {#if error}<div
