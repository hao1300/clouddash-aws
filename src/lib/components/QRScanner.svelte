...
<div
    class="relative w-full aspect-square overflow-hidden rounded-lg bg-black border border-gray-800 shadow-inner group"
>
    ...
    {#if !isStarted && !errorMsg}
        ...
        >
            <div
                class="w-10 h-10 border-4 border-blue-500/20 border-t-blue-500 rounded-full animate-spin mb-3"
            ></div>
    ...
    {#if errorMsg}
        <div
            class="absolute inset-0 flex flex-col items-center justify-center p-6 text-center bg-gray-900"
        >
            <svg
    ...
    {#if isStarted}
        ...
        >
            <div class="relative w-[300px] h-[300px]">
                ...
                <!-- Scanning Line -->
                <div
                    class="absolute top-0 left-0 w-full h-0.5 bg-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.8)] animate-scan"
                ></div>
            </div>
        </div>
        <div
        ...
        >
            <span
                class="text-[10px] text-white/50 bg-black/40 px-3 py-1 rounded-full backdrop-blur-sm uppercase tracking-widest font-bold"
                >Align QR Code within frame</span
            >
        </div>
    {/if}
</div>
